/*********************************************************************
 *
 *                  Power API definitions  <<< ??? Say What ?
 *
 *********************************************************************
 * FileName:        power.h  <<< ??? Say What ?
 * Dependencies:
 * Processor:     PIC32
 *
 *
...
void OSCConfig(unsigned long int source, unsigned long int mult, unsigned long int post, unsigned long int div)
{
	...
	// DRN: No unlock performed here...  unlock OSSCON register,
	OSCCONCLR = _OSCCON_NOSC_MASK; // set new clock source to FRC (w/out divider, PLL, etc)
	...
	while ( _OSCCON_OSWEN_MASK & OSCCON) {}; 	// wait for the clock switch to newly requested clock
