...
#if defined(PICC18PORT)
	volatile CDC_NOTICE cdc_notice @ 0x240;
	#if defined(__18F14K50)
		// These need to be in USB RAM and are 64 bytes each
		volatile unsigned char cdc_data_rx[CDC_DATA_OUT_EP_SIZE] @ 0x250;
		volatile unsigned char cdc_data_tx[CDC_DATA_IN_EP_SIZE] @ 0x258;
                LINE_CODING line_coding @ 0x248;    // Buffer to store line coding infor
	#else
		// These need to be in USB RAM and are 64 bytes each
		volatile unsigned char cdc_data_rx[CDC_DATA_OUT_EP_SIZE] @ 0x604;
		volatile unsigned char cdc_data_tx[CDC_DATA_IN_EP_SIZE] @ 0x646;
